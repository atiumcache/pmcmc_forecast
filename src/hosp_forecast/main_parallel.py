 ...
 def main(forecasted_betas: Array, location_code: str, reference_date: str, use_nbinom: bool = True) -> None:                        
     ...
     with mp.Pool(mp.cpu_count() - 2) as pool:
         results = pool.starmap(                                                                                                     
             solve_for_bootstrap,                                                                                                    
             [(i, forecasted_betas, all_data, forecast_span, endpoint) for i in range(num_bootstraps)]                               
         )                                                                                                                           
     ...
     if use_nbinom:                                                                                                                  
         ...
         time_series = np.copy(                                                                                                      
             np.concatenate(                                                                                                         
                 (all_data.observations[: time_span[1]].squeeze(), forecast_new_hosp.mean(axis=0))                                   
             )                                                                                                                       
         )                                                                                                                           
         ...
         quantiles_hosp = [                                                                                                          
             calculate_quantiles(forecast_new_hosp[:, i]) for i in range(forecast_new_hosp.shape[1])                                 
         ]                                                                                                                           
 ...
 class DataReader:                                                                                                                   
     ...
     def read_in_data(self):
         predicted_beta_path = os.path.join(paths.OUTPUT_DIR, 'trend_forecast_test',
 ...
 def solve_for_bootstrap(i, forecasted_betas, all_data, forecast_span, endpoint):                                                    
     def functional_beta(t):                                                                                                         
         ...
         if t < forecast_span[0]:                                                                                                    
             ...
             beta_to_return = forecasted_betas[i, t - endpoint - 1]                                                                  
