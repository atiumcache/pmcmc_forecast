...
def plot_predictions_with_quantile_range(prediction_date, location, weeks_prior=4): 
    pred_path = os.path.join(paths.HOSP_OUTPUT_DIR, prediction_date, f'{location}-PMCMC-flu-predictions.csv')
    ...
    prediction_data['target_end_date'] = pd.to_datetime(prediction_data['target_end_date'])
    prediction_data['reference_date'] = pd.to_datetime(prediction_data['reference_date'])
    ...
    actual_subset = actual_data_filtered[(actual_data_filtered['date'] >= start_date) & (actual_data_filtered['date'] <= prediction_date + pd.DateOffset(weeks=4))]
    
    # Get predictions for the same location and prediction date
    prediction_subset = prediction_data[(prediction_data['reference_date'] == prediction_date)]
    
    # Select relevant quantiles directly
    lower_quantile_data = prediction_subset[prediction_subset['output_type_id'] == 0.05]
    median_quantile_data = prediction_subset[prediction_subset['output_type_id'] == 0.50]
    upper_quantile_data = prediction_subset[prediction_subset['output_type_id'] == 0.95]    # Resample actual data to weekly sums
    ...
    ax.plot(actual_weekly['date'], actual_weekly['previous_day_admission_influenza_confirmed'], 
            label='Actual Hospitalizations (Weekly Sum)', color='black', marker='o')

    actual_on_pred_date = actual_weekly.loc[actual_weekly['date'] == prediction_date, 'previous_day_admission_influenza_confirmed'].values[0]

    lower_quantile_data = lower_quantile_data.drop(columns=['horizon', 'location', 'output_type_id', 'output_type', 'reference_date'])
    new_row = {
    ...
    ax.plot(median_quantile_data['target_end_date'], median_quantile_data['value'], label='50th Percentile (Median)', color='blue')
    ax.plot(median_quantile_data['target_end_date'], median_quantile_data['value'], 's', color='blue', markersize=6)
    one_week_ahead = prediction_date + pd.DateOffset(weeks=1)
    ax.plot([prediction_date, one_week_ahead], 
            [actual_on_pred_date,
             median_quantile_data.loc[median_quantile_data['target_end_date'] == one_week_ahead, 'value'].values[0]])          
    ...
    plt.title(f'Actual vs Predicted Hospitalizations | Location {location} | Forecast Date: {prediction_date.strftime("%Y-%m-%d")}')
